...
第一步 API 先创建 `pending` 状态的 `upload_task`，记录原始文件名、大小、创建人和 `raw_upload_path`。`complete` API 将 `pending` 任务推进到 `processing`，并记录 `started_at`，后续由 release 创建和检测 job 接管。真实 zip 内容接收、文件校验和检测执行在后续步骤接入。
