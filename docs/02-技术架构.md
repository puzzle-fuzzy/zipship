...
第一步 API 先创建 `pending` 状态的 `upload_task`，记录原始文件名、大小、创建人和 `raw_upload_path`。`complete` API 将 `pending` 任务推进到 `processing`，记录 `started_at`，创建 `processing` 状态的 release，并把 `upload_task.release_id` 指向该 release。真实 zip 内容接收、文件校验、真实 hash 计算和检测执行在后续步骤接入。
