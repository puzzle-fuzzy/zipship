...
- 第 1～3 项已完成：Workspace 与基础设施、全新数据库模型、认证、个人组织、成员查询和项目创建/查询均已进入 Rust 实现。
- PostgreSQL 事务测试已经覆盖注册时组织创建、角色隔离、项目审计及并发 Slug 冲突；实际 HTTP 进程 smoke 已贯通注册到项目详情。
- 下一项进入第 4 项：流式 ZIP 上传、接收大小约束、staging 生命周期和持久化处理 Job。
