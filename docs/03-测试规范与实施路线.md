...
> 历史状态：本文记录第一阶段 Elysia/Drizzle/Nginx 测试路线，仅用于追溯。当前质量门由 Rust workspace 测试、真实 PostgreSQL/SMTP/HTTP 测试、Rust OpenAPI 漂移检查和 Console Vitest 组成，执行入口见根 `README.md` 与 `.github/workflows/ci.yml`。
...
@elysia/eden   API 集成测试与前端类型安全客户端
...
API 测试不直接拼接 URL 和手写响应类型。`apps/api` 导出 `type App = typeof app`，`packages/api-client` 使用 `@elysia/eden` 创建 `treaty<App>()` 客户端。Web Console 和 Desktop Shell 通过该客户端调用接口；API 单元测试和轻量集成测试优先把 Elysia app 实例直接传给 Eden Treaty，避免真实网络请求，同时保留端到端类型安全。

API 错误测试只断言稳定错误码和结构化字段，不断言中文或英文展示文案。用户可见文案由前端 i18n 字典负责，中英文切换测试应覆盖 Web Shell 与 Desktop Shell 的 UI 文案映射，而不是要求后端为同一个错误返回两套语言。
...
```txt
1. 从 apps/api 导入 app 实例和 App 类型。
2. 优先使用 @elysia/eden 的 treaty(app) 创建无网络测试客户端；需要跨进程测试时再使用 treaty<App>(baseUrl)。
3. 通过 Eden 客户端调用 /_api/* 路由。
4. 只在断言 HTTP 边界行为时检查状态码；业务字段优先使用 Eden 推导出的响应类型。
5. 错误响应断言稳定 code，例如 DUPLICATE_EMAIL，不断言展示文案。
```
...
所有跨 workspace 重复依赖必须在根 `package.json` 的 Bun `catalog` 中声明，并在子包使用 `catalog:`，尤其是 `elysia`、`@elysia/eden`、`typescript`、`vite`、`react`、`electron` 和 Drizzle 相关依赖。
...
测试环境变量统一写在仓库根 `.env.test` 或由 CI 注入，不在 `apps/*` 或 `packages/*` 下各自维护一份 env。前端测试需要的公开变量使用 `VITE_` 前缀；数据库、密钥、存储目录等服务端变量只能被后端、脚本和测试进程读取。
...
VITE_ZIPSHIP_API_BASE_URL=http://127.0.0.1:3001
```
