...
**Root `package.json` `scripts`** — add (matching Turborepo's official oxlint guide):
- `"lint": "oxlint ."`
- `"lint:fix": "oxlint --fix ."`
...
**`turbo.json` `tasks`** — register the scripts as **Root Tasks** (the `//#` prefix), per Turborepo's official oxlint guide:
```json
"//#lint": {},
"//#lint:fix": {
  "cache": false
...
```
`//#lint` runs the root `oxlint .` exactly once for the whole repo. `lint:fix` mutates files, so it must be `cache: false`. Sub-packages get **no** `lint` script of their own — a single root config covers the monorepo.

**Caching tradeoff (acknowledged):** Turborepo warns that root-level oxlint invalidates the cache for *all* tasks whenever the oxlint version or the root config changes. For a monorepo of ZipShip's size this is acceptable and matches the official recommendation. If cache-hit ratio ever matters more than config simplicity, the documented fallback is per-package lint tasks with `dependsOn: ["^build"]` — explicitly not chosen here.
...
- No per-package oxlint configs — one root config covers the monorepo (industry best practice, confirmed against the oxlint and Turborepo docs). If a subtree ever needs different rules, the idiomatic fix is an `overrides` block scoped by glob (e.g. `apps/api/**`) inside the **root** `.oxlintrc.json` — NOT a nested config file. Nested oxlint configs do **not** auto-merge with the parent (the child must explicitly `extends: ["../.oxlintrc.json"]`), so per-package files easily cause silent rule loss and config drift.
...
- `turbo.json` — add `//#lint` and `//#lint:fix` Root Tasks.
