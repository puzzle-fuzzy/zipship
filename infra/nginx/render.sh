...
#
# Usage:
#   ZIPSHIP_API_UPSTREAM=http://api:3001 \
...
envsubst \
  '${ZIPSHIP_LISTEN_PORT} ${ZIPSHIP_MAX_BODY_SIZE} ${ZIPSHIP_API_UPSTREAM} ${ZIPSHIP_SITES_ROOT} ${ZIPSHIP_CONSOLE_ROOT} ${ZIPSHIP_NGINX_PID}' \
  < "$TEMPLATE" \
  > "$OUTPUT"
